...
class DescentTask(Task):
    ...
    @staticmethod
    def _parse_logtargets(stdout):
        logtargets = []
        target = None
        for line in stdout.splitlines():
            target_match = re.match(r'# target=(.*)', line)
            if target_match:
                target = target_match.group(1)
            match = re.match(r'log\(target\)=(\d+)', line)
            if match:
                logtargets.append((target, int(match.group(1))))
                target = None
        return logtargets

    def _make_descent_program(self, target, stdoutpath, stderrpath,
                              threads=None):
        program_args = dict(self.merged_args[0])
        program_args["threads"] = threads
    ...
    def _handle_descent_result(self, targets, message):
        if message.get_exitcode(0) != 0:
            self.log_failed_command_error(message, 0)
            raise Exception("Program failed")

        stdout = message.read_stdout(0).decode()
        logtargets = self._parse_logtargets(stdout)
        for index, (printed_target, logtarget) in enumerate(logtargets):
            target = printed_target
            if target is None:
                target = targets[index]
            elif self.params.get("gfpext", 1) == 1:
                target = int(target)
    ...
    def run(self):
        ...
        extdeg = self.params.get("gfpext", 1) or 1
        target_threads = self.params.get("threads")
        if target_threads is None:
            target_threads = self.parameters.parameters.get_or_set_default(
                "tasks.threads", 1)
        if extdeg == 1:
            targets = [int(ts) for ts in target.split(",")]
        else:
            targets = target.split(";")
        worker_count = max(1, min(int(target_threads), len(targets)))
        self.logger.info("Running %d target descent(s) with up to %d thread(s)",
        ...
        message = self.submit_command(p, None, log_errors=True)
        self._handle_descent_result(targets, message)
        return True
